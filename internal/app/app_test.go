...
func TestMenuModalRect_Centered(t *testing.T) {
	a := newTestApp(t, t.TempDir())
	// The menu (121 rows fully expanded) outgrew the 40-row default sim
...
// Quit renders headerless behind a divider (its 1 row) — 121 total. The
...
func TestMenuLayout_NoCustomActions(t *testing.T) {
	...
	if got := len(items); got != 122 {
		t.Errorf("row count = %d, want 122 (2 top-zone + 106 group actions + 14 headers)", got)
	}
...
func TestMenuLayout_WithCustomActions(t *testing.T) {
	...
	if h != 131 { // 128 baseline + custom header + 2 items
		t.Errorf("modalHeight = %d, want 131", h)
	}
