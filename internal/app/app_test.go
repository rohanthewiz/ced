...
func TestMenuLayout_NoCustomActions(t *testing.T) {
	...
	if got := len(items); got != 159 {
		t.Errorf("row count = %d, want 159 (2 top-zone + 142 group actions + 15 headers)", got)
	}
	// The pinned title divider (2), the one under the top zone (5), and the
	// one setting off the headerless Quit group (162) — headers separate the
	// rest.
	wantDiv := []int{2, 5, 162}
...
func TestMenuLayout_WithCustomActions(t *testing.T) {
	...
	if h != 168 { // 165 baseline + custom header + 2 items
		t.Errorf("modalHeight = %d, want 168", h)
	}
