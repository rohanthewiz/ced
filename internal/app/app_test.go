...
func TestMenuLayout_NoCustomActions(t *testing.T) {
	...
	if got := len(items); got != 162 {
		t.Errorf("row count = %d, want 162 (2 top-zone + 145 group actions + 15 headers)", got)
	}
	// The pinned title divider (2), the one under the top zone (5), and the
	// one setting off the headerless Quit group (165) — headers separate the
	// rest.
	wantDiv := []int{2, 5, 165}
...
func TestMenuLayout_WithCustomActions(t *testing.T) {
	...
	if h != 171 { // 168 baseline + custom header + 2 items
