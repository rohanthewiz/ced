...
func TestMenuModalRect_Centered(t *testing.T) {
	a := newTestApp(t, t.TempDir())
	// The menu (117 rows fully expanded) outgrew the 40-row default sim
...
// Quit renders headerless behind a divider (its 1 row) — 117 total. The
...
func TestMenuLayout_NoCustomActions(t *testing.T) {
	...
	if got := len(items); got != 117 {
		t.Errorf("row count = %d, want 117 (2 top-zone + 101 group actions + 14 headers)", got)
	}
...
func TestMenuLayout_WithCustomActions(t *testing.T) {
	...
	if h != 126 { // 123 baseline + custom header + 2 items
