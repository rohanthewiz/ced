...
// Quit renders headerless behind a divider (its 1 row) — 143 total. The
...
func TestMenuLayout_NoCustomActions(t *testing.T) {
	...
	if h != 149 {
		t.Errorf("modalHeight = %d, want 149", h)
	}
	if got := len(items); got != 143 {
		t.Errorf("row count = %d, want 143 (2 top-zone + 126 group actions + 15 headers)", got)
	}
	// The pinned title divider (2), the one under the top zone (5), and the
	// one setting off the headerless Quit group (146) — headers separate the
	// rest.
	wantDiv := []int{2, 5, 146}
...
func TestMenuLayout_WithCustomActions(t *testing.T) {
	...
	if h != 152 { // 149 baseline + custom header + 2 items
		t.Errorf("modalHeight = %d, want 152", h)
	}
