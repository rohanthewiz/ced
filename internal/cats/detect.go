// =============================================================================
// File: internal/cats/detect.go
// Author: Rohan Allison <rohanthewiz@gmail.com>
// Created: 2026-08-12
// Copyright: 2026 Rohan Allison. All rights reserved.
// =============================================================================

// Package cats is ced's client for the cats terminal multiplexer it usually
// runs inside: the control socket (one JSON request per line, one response
// back), the event stream that socket can be upgraded to, and the separate
// hook socket an agent reports its state on.
//
// It is shaped like internal/remote and internal/lsp — a unix-socket peer
// speaking ndjson, stdlib only — and it obeys the same two house rules:
//
//   - EVENTS ONLY. Nothing in here touches App state. Background goroutines
//     hand results to callbacks whose only job is to PostEvent; the main loop
//     is the sole mutator. See internal/app/cats_glue.go.
...
//     ced behaves exactly as it does in any other terminal. No feature may
//     exist only at Tier 1 (ai_docs/cats-native-plan.md §2).
//
// THE WIRE STRUCTS ARE HAND-COPIED MIRRORS of cats' own
// (internal/app/command_vocab.go, internal/app/events.go,
// cmd/catway/hooks.go), deliberately minimal — only the fields ced reads or
// sends. ced must NOT import the cats module: it has to stay buildable and
// installable on a machine that has never heard of cats, and a shared type
// package would make the editor's build depend on the multiplexer's.
//
...
//	             it for ProbeTimeout — so the app runs it on a goroutine and
...
// purpose, and for the same reason internal/remote's probeTimeout is: the
// socket may belong to a wedged server, and the right answer to that is to
// fall back to Tier 0 quickly, not to diagnose it. A package var so tests
// can collapse the window rather than sleep through it.
...
type Caps struct {
	...
	// Service and Protocol are the ping reply's self-identification, kept
	// for the status row and for future protocol gating. The service name
	// is deliberately NOT matched against a constant: catway answers
	// "catway", the Rust cats answers its own name, and a client that
	// rejected an unfamiliar one would break on the sibling implementation
	// it is meant to work with.
	Service  string
	Protocol int

	// Reason explains a Tier-0 verdict in one phrase for the ≡ row ("not
	// running inside cats", "control socket not answering"). Empty at
	// Tier 1. Nothing branches on it — it exists so the degradation is
	// silent but not INVISIBLE when a user goes looking for why.
	Reason string
}
...
// callers that can afford the IO (tests, a future `ced --cats-status`). The
// app deliberately does not use it; see the package comment.
...
// dial opens a unix connection with the probe timeout applied to both the
