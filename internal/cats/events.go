...
// connection until somebody hangs up — so ced learns that a sibling agent
// went from working to blocked, that focus moved, or that a pane died,
// without polling for any of it.
//
...
//     state. It is expected to do exactly one thing: PostEvent. That is the
//     iron events-only rule (see the package comment).
...
const (
	...
	EventPaneTitle    = "pane_title"
	EventPaneCwd      = "pane_cwd"
...
// back in a second; the cap keeps a permanently-dead socket down to two
// wakeups a minute. No jitter: there is exactly one subscriber per editor,
// so there is no thundering herd to spread out, and a deterministic sequence
// is one a test can assert on.
//
// Package vars for the same single reason internal/editor's SyntaxSettle is
// one — a test collapses the window instead of sleeping through it. They are
// not settings.
var (
...
// PaneAgentEvent is EventPaneAgent's payload: a pane's detected agent
// identity or activity state changed. State is idle|working|blocked|unknown;
// Agent is "" for a plain shell.
...
// PaneTextEvent is the shared payload of EventPaneTitle and EventPaneCwd —
// a pane plus the one string it just reported. Two events, one shape; the
// field is named for what both carry rather than duplicating the struct.
type PaneTextEvent struct {
	Pane  uint32 `json:"pane"`
	Title string `json:"title,omitempty"`
	Cwd   string `json:"cwd,omitempty"`
}

// PaneExitedEvent is EventPaneExited's payload.
type PaneExitedEvent struct {
	Pane     uint32 `json:"pane"`
	ExitCode int    `json:"exit_code"`
}

// PaneRefEvent is the payload of the three structural events (added,
// removed, focus_changed): they name a pane and nothing more. For
// focus_changed, Pane is the newly focused pane (0 for none).
...
// optional: no pane means every pane, no events means every name. ced
// subscribes to the names it acts on rather than filtering client-side, so
// an idle editor is not woken by traffic it would only discard.
...
// connection comes up or goes down — the editor uses it to keep its Tier
// indicator honest, since the stream is the only thing that notices a cats
// that died AFTER startup detection said Tier 1.
//
// Both callbacks run on the reader goroutine. Neither may touch App state.
...
// tear one down during shutdown without racing a callback that posts to a
// screen that is being finalized. Safe to call twice, and on a nil Stream
// (the Tier-0 case, where nothing was ever subscribed).
func (s *Stream) Close() {
	if s == nil {
		return
	}
...
// run is the reconnect loop: connect, pump until the connection dies, wait
// out the backoff, repeat — until Close.
func (s *Stream) run() {
	defer close(s.done)
	delay := reconnectMin
	for {
...
// the connection fails. It returns nil when the server hung up cleanly and
// an error when the connect or handshake failed.
func (s *Stream) session() error {
	...
	// Publish the connection so Close can interrupt the read below, and
	// bail out if Close already happened while we were shaking hands.
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return nil
	...
	s.state(true, nil)
	// Rule 1: no deadline on the pump. dial set one for the handshake;
	// clear it now or the first quiet minute would look like a failure.
...
// deadline. A refused subscription (unknown event name, streaming disabled)
// is an error like any other — it reconnects, which is right: the server
// that refuses today may be a different build tomorrow.
func (s *Stream) handshake(conn net.Conn, dec *json.Decoder) error {
	line, err := json.Marshal(request{ID: "ced-events", Method: MethodSubscribe, Params: s.filter})
	if err != nil {
		return err
	}
...
// this transport, so an unnamed object is something new the server sends
// that this build predates, and ignoring it is forward compatibility.
...
// subscribeError is a server-side refusal of the subscription, distinct from
// a transport failure only in its message.
