...
//
//	$XDG_CONFIG_HOME/ced/format-defaults.json   (preferred)
//	~/.config/ced/format-defaults.json          (fallback)
//
...
// .ced/format.json on consent). That's the only time defaults
...
var defaultsPathEnv = "CED_DEFAULTS_FILE"

// DefaultsPath returns the canonical defaults-file location:
// $XDG_CONFIG_HOME/ced/format-defaults.json, falling back to
// ~/.config/ced/format-defaults.json. Returns "" when neither
// resolves — the caller treats that as "no defaults file possible"
// and skips the install-prompt feature entirely.
//
// Tests can override the path via CED_DEFAULTS_FILE.
func DefaultsPath() string {
	if override := os.Getenv(defaultsPathEnv); override != "" {
		return override
	}
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "ced", DefaultsFile)
	}
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".config", "ced", DefaultsFile)
}
...
// .ced directory and the file if either is missing. Writes
