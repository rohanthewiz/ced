...
// OK with this project's .ced/format.json running its
...
var trustFileEnv = "CED_TRUST_FILE"

// DefaultTrustPath returns the canonical trust-file location:
// $XDG_CONFIG_HOME/ced/format-trust.json, falling back to
// ~/.config/ced/format-trust.json. Returns "" when neither
// resolves — callers treat that as "no persistent trust available"
// (every save will re-prompt, which is annoying but safe).
//
// Tests can override the location by setting CED_TRUST_FILE.
func DefaultTrustPath() string {
	if override := os.Getenv(trustFileEnv); override != "" {
		return override
	}
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "ced", "format-trust.json")
	}
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return ""
	}
	return filepath.Join(home, ".config", "ced", "format-trust.json")
}
